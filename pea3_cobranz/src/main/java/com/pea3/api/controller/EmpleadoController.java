...
import com.pea3.api.service.EmpleadoService;
...
public class EmpleadoController {
	...
	@Autowired
	EmpleadoService pagoServicio;
	
	@RequestMapping(value = "/", method = RequestMethod.GET)
	public ResponseEntity<List<com.pea3.api.model.Empleado>> getCliente(){
		List<Empleado>empleado=pagoServicio.ListAllEmpleado();
	...
	public ResponseEntity<Empleado> getEmpleado(@PathVariable("id") Long id){
		...
		Empleado pago = pagoServicio.getEmpleado(id);
		if(pago == null) {
	...
	@PostMapping
	public ResponseEntity<Empleado> CrearPago(@Valid @RequestBody Empleado pago,BindingResult result){
		
		if(result.hasErrors()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,this.formatMessage(result));
		}

		Empleado pagoCreado = pagoServicio.createEmpleado(pago);
		
		return ResponseEntity.status(HttpStatus.CREATED).body(pagoCreado);
	}

	@RequestMapping(value = "/{id}",method = RequestMethod.PUT)
	public ResponseEntity<Empleado> actualizarPago(@PathVariable("id") Long id,
			@RequestBody Empleado pago){
		
		pago.setIdempleado(id);
		Empleado pagoEncontrado = pagoServicio.updateEmpleado(pago);
	...
	public ResponseEntity<Empleado> deletePago(@PathVariable("id") Long id){
		Empleado pagoDelete = pagoServicio.deleteEmpleado(id);
